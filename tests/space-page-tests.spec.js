...
const moreSpaceLink = 'https://unsplash.com/s/photos/space';
...
test.beforeEach('Run the Space Page', async ({ page }) => {
  await page.goto(spacePageUrl);
});

test('The Space page is running', async ({ page }) => {
  expect(page.url()).toBe(spacePageUrl);
});

test('Space Page: Page Heading exists', async ({ page }) => {
  await expect(page.getByRole('heading', { name: 'Space' })).toBeAttached();
});
...
test('Space Page: The header container should be a flex container, if layout.css is connected', async ({
  page,
}) => {
...
test("Space Page: The 'image-details' should a flex container, if the image-list.css is connected", async ({
  page,
}) => {
...
test('Space Page: JavaScript file relative path points to an existing JS files', async ({
  page,
}) => {
...
test('Space Page: the scripts.js file should be connected', async ({
  page,
}) => {
  const logs = [];
...
test('Space Page: there should be no error messages in the console', async ({
  page,
}) => {
  const errors = [];
...
test('Space Page: The space-image.jpg image file path is used for the main image', async ({
  page,
}) => {
