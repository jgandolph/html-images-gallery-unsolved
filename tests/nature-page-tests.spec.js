...
const moreNatureLink = 'https://unsplash.com/s/photos/nature';
...
test.beforeEach('Run the Nature Page', async ({ page }) => {
  await page.goto(naturePageUrl);
});

test('The Nature page is running', async ({ page }) => {
  expect(page.url()).toBe(naturePageUrl);
});

test('Nature Page: Page Heading exists', async ({ page }) => {
  await expect(page.getByRole('heading', { name: 'Nature' })).toBeAttached();
});

// CSS connection tests

test('Nature Page: layout.css should be connected', async ({ page }) => {
...
test('Nature Page: image-detail.css should be connected', async ({ page }) => {
  const cssLocator = page.locator('[rel="stylesheet"]');
...
test('Nature Page: The header container should be a flex container, if layout.css is connected', async ({
  page,
}) => {
...
test("Nature Page: The 'image-details' should a flex container, if the image-list.css is connected", async ({
  page,
}) => {
...
test('Nature Page: JavaScript file relative path points to an existing JS files', async ({
  page,
}) => {
...
test('Nature Page: the scripts.js file should be connected', async ({
  page,
}) => {
  const logs = [];
...
test('Nature Page: there should be no error messages in the console', async ({
  page,
}) => {
  const errors = [];
...
test('Nature Page: The nature-image.jpg image file path is used for the main image', async ({
  page,
}) => {
...
test('Nature Page: Should open the space page when clicked in the top navigation', async ({
  page,
}) => {
  await page
    .locator('nav')
...
test('Nature Page: Should open the plants page when clicked in the top navigation', async ({
  page,
}) => {
  await page
    .locator('nav')
...
test('Nature Page: Should open the modern design page when clicked in the top navigation', async ({
  page,
}) => {
  await page
    .locator('nav')
...
test('"Prev: Plants" link should redirect to the Plants page', async ({
  page,
}) => {
  await page
    .getByRole('link', { name: 'Prev: Plants' })
    .click();
  expect(page.url()).toBe(plantsPageUrl);
});

test('"Next: Space" link should redirect to the Space page', async ({
  page,
}) => {
  await page
    .getByRole('link', { name: 'Next: Space' })
    .click();
  expect(page.url()).toBe(spacePageUrl);
});
...
test('"More Modern Design Images" link should use an absolute path to unsplash.com "modern design" collection page', async ({
  ...
  await page
    .getByRole('link', { name: 'More Nature Images' })
    .click();
  expect(page.url() === moreNatureLink).toBeTruthy();
});
